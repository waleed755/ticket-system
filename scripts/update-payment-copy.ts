// One-time content fix: rewrite JazzCash/card-specific wording across legal
// pages to describe the new manual bank-transfer + screenshot verification
// flow, and remove the phone number from the Contact & Support paragraph
// (the phone number the checkout WhatsApp link uses is unrelated and unchanged).
...
How tickets are delivered: once a payment is successfully verified by our team, Ticket Buddy automatically generates one unique digital ticket per attendee, each with its own scannable QR code. These tickets are:
...
Delivery time: digital tickets are generated and emailed within moments of payment verification. If a ticket email does not arrive within 15 minutes, check your spam folder, then use the "Resend tickets" option in your account dashboard or contact us at tickets@ticketbuddy.pk.
...
- Payments are made by bank transfer or QR code to the account details provided at checkout, and are manually verified by our team before a booking is confirmed.
- Ticket Buddy is operated from 604 N Block, Samanabad, Lahore, Pakistan. For questions about these terms, contact tickets@ticketbuddy.pk. `,
    },
  });
...
Payment information: Ticket Buddy does not collect, store, or process your card, bank account, or wallet credentials. To pay for a booking, you transfer payment directly to the account details (or QR code) provided at checkout and upload a screenshot as proof of payment, which our team reviews manually to verify and confirm your booking.
...
Business contact: Ticket Buddy, 604 N Block, Samanabad, Lahore, Pakistan — tickets@ticketbuddy.pk — 0347 6581443 / 0304 1549196. `,
    },
  });
...
Our business model: organizers list their events (concerts, conferences, comedy nights, and more) on Ticket Buddy with full details — date, time, venue, ticket categories, and pricing in PKR. Customers browse events, select ticket categories, enter attendee details, and pay by bank transfer or QR code, uploading a screenshot as proof of payment. Once payment is verified by our team, a unique digital ticket with a scannable code is generated for every attendee and delivered by email, and is always available for download from the customer's account.
...
Refund timeline: approved refunds are returned via bank transfer to the account used for payment, typically within 3–7 business days of approval.
...
Questions about a specific refund? Contact us at tickets@ticketbuddy.pk or 0347 6581443 / 0304 1549196.`,
    },
  });

  await prisma.siteFAQ.updateMany({
    where: { question: "What payment methods do you accept?" },
    data: {
      question: "How can I pay for my tickets?",
      answer:
        "After selecting your tickets and completing your booking details, you will see our payment details and QR code at checkout. Make the payment using the provided details or scan the QR code, then upload a screenshot of your payment. Once verified, we will confirm your booking and send your ticket confirmation by email.",
    },
  });

  console.log("Payment copy updated.");
}
